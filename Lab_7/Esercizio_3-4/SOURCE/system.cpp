...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        distance(2) = this->pbc( _particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);
        dr = sqrt( dot(distance,distance) );
        if(dr < _halfside.min()) _measurement(floor(dr/_bin_size)) += 2;
        if(dr < _r_cut){
  ...
  if (_measure_pressure){
    // MC :
    //pressure_temp = _ptail + _rho * _temp + 48. * pressure_temp / (3*_volume);
    //NVE :
...
void System :: averages(int blk){
  ...
  if (_measure_gofr && blk==_nblocks){
    ...
    double bin = _index_gofr;
    while (bin < _n_bins)
    {
      sum_average = _global_av(bin)/(4*M_PI/3*(pow((bin+1)*_bin_size, 3) - pow(bin*_bin_size, 3)));
      sum_ave2 = _global_av2(bin)/(pow(4*M_PI/3*(pow((bin+1)*_bin_size, 3) - pow(bin*_bin_size, 3)),2));
      coutf << setw(20) << bin*_bin_size
          << setw(20) << sum_average/double(blk)
          << setw(20) << this->error(sum_average, sum_ave2, blk) << endl;
      bin += 1;
    }
