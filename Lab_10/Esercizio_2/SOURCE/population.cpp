...
void Population::initialize(int continent){
  ...
  for(int i=0; i<12; i++){
    ...
    filePath = "../OUTPUT/CONTINENT" + std::to_string(i) + "/Generations.dat";
    ofstream coutp(filePath);
    coutp << "#     GEN:  SHORTEST LENGTH: " << endl;
    coutp.close();

    filePath = "../OUTPUT/CONTINENT" + std::to_string(i) + "/Generations_ave.dat";
    ofstream couta(filePath);
    couta << "#     GEN:  SHORTEST LENGTH: " << endl;
    couta.close();

  filePath = "../OUTPUT/CONTINENT" + std::to_string(i) + "/Sequences.dat";
    ofstream coutb(filePath);
    coutb << "#     SEQUENCE: " << endl;
    coutb.close();
  }
   ...
   //input cities
  ifstream input2;
  input2.open("../INPUT/cap_prov_ita.dat");
  double x=0, y=0;
  int counter=0;
  while ( !input2.eof() ){
    input2 >> x >>y;
...
int Population::get_random_number(double num1, double num2){
  return int(_rnd.Rannyu(num1, num2+1));
}
...
void Population::print(int continent){
  ...
  filePath = "../OUTPUT/CONTINENT" + std::to_string(continent) + "/Generations.dat";
  ...
  _length_ave=0;
  filePath = "../OUTPUT/CONTINENT" + std::to_string(continent) + "/Generations_ave.dat";
  coutf.open(filePath,ios::app);
  ...
  filePath = "../OUTPUT/CONTINENT" + std::to_string(continent) + "/Sequences.dat";
