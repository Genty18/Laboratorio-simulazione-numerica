...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    double k = _rnd.Rannyu();
    int sum = _particle(this->pbc(i-1)).getspin() +_particle(this->pbc(i+1)).getspin();
    double flip_prob = 1/(1+exp(-2*_beta*(_J*sum + _H)));
    if(k < flip_prob) _particle(i).setspin(1);
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      //EVENTUALMENTE AGIRE QUI PER CONDIZIONI INIZIALI DIFFERENTI
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_pressure){
    pressure_temp = _ptail + _rho * _measurement(_index_temp) + 48. * pressure_temp / (3*_volume);
    _measurement(_index_pressure) = pressure_temp;
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet){
    for(int p=0; p<_npart;p++) magnetization+=_particle(p).getspin();
    _measurement(_index_magnet) = magnetization;
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv){
  _measurement(_index_cv) =  pow(_npart*_measurement(_index_tenergy),2);
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
...
void System :: averages(int blk){
  ...
  double sum_ave2, sum_average, average;
